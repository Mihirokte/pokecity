...
function CatanScene({ entries, onSelectResident, onAddClick }: CatanSceneProps) {
  ...
  useEffect(() => {
    ...
    entries.forEach(({ house }) => {
      const houseConfig = require('../../config/houseTypes').HOUSE_TYPES[house.type];
        ...
        ROADS.slice(0, entries.length - 1).map(([fromIdx, toIdx], roadIdx) => {
          ...
          return (
            <Road
              ...
              to={[toX, toZ]}
              roadIndex={roadIdx}
              fromColor={require('../../config/houseTypes').HOUSE_TYPES[fromEntry.house.type].color}
              toColor={require('../../config/houseTypes').HOUSE_TYPES[toEntry.house.type].color}
            />
          );
        })}
